...
class AdminController extends PermsController
{
	...
	public function GenerateGoogleAccessCodeAction() {
		/** @var GoogleApi_Helper */
		$helper = DI::getDefault()->get('googleApiHelper');

		if (isset($_GET['code'])) {
			try {
				$helper->generateAccessToken($_GET['code']);
				SimpleAlert::success('<i class="far fa-grin-beam"></i> Google Drive Integrated Successfully!', true);
