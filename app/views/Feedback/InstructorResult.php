				...
				<tr>
					...
					<td>
						<div class="response-percent">
							<span class="percent"><?php echo $totalResponses == 0 ? 0 : round($results['responders'] / $totalResponses * 100, 1) ?></span> responded to this question
						</div>
						...
						<?php if ($field->type == FormFieldTypeEnum::RATING()) : ?>
							<div class="rating-picks mb-2">
								...
								<?php for ($i = 1; $i <= 5; $i++) : $sum += $results['data'][$i] * $i ?>
									<div class="response-percent">
										<span class="percent"><?php echo $results['responders'] == 0 ? 0 : round($results['data'][$i] / $results['responders'] * 100, 1) ?></span>
										<?php for ($j = 0; $j < $i; $j++) : ?>
						...
						<?php else : ?>
							<div class="option-picks">
								<?php foreach ($field->options as $ndx => $label) : ?>
									<div class="response-percent">
										<span class="percent"><?php echo $results['responders'] == 0 ? 0 : round($results['data'][$ndx] / $results['responders'] * 100, 1) ?></span> <?php echo $label ?>
									</div>
								<?php endforeach; ?>
							</div>
						<?php endif; ?>
